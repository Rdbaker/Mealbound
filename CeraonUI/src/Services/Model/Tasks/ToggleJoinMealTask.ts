...
export class ToggleJoinMealTask extends ModelTask<ModelUpdateResult<Meal>> {
  constructor(private _api: ICeraonModelAPI, private _id: string, private _join: boolean, startLoading: boolean) {
    super(startLoading);
  }

  run(next?: (task: ModelTask<ModelUpdateResult<Meal>>, result: ModelUpdateResult<Meal>)=>void) {
    this._api.toggleJoinMeal(this._id, this._join).then((result: ModelUpdateResult<Meal>) => {
